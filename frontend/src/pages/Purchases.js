...
const Purchases = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setShowModal(false);
      setFormData({ supplier_id: '', items: [{ product_id: '', quantity: 1 }] });
      fetchPurchases();
    } catch (error) {
    ...
    <div className="crm-page">
      <div className="page-header">
        <div>
          ...
          <p className="page-subtitle">Administrer innkjøpsordrer</p>
        </div>
          ...
          <tbody>
            {purchases.map(purchase => (
              <tr key={purchase.id}>
                ...
                <td>{purchase.items.length} produkt(er)</td>
                <td>{Math.round(purchase.total)} kr</td>
                <td>
                  <span className={`badge badge-${purchase.status === 'received' ? 'success' : 'warning'}`}>
                    {purchase.status === 'received' ? 'Mottatt' : 'Venter'}
                  </span>
                </td>
                <td>{new Date(purchase.order_date).toLocaleDateString('nb-NO')}</td>
                <td>
